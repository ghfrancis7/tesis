<?php 
	...
	include_once("../../modelo/Producto.php");
		...
		$pdf=new PDF_MC_Table('L', 'mm', 'A4');
			...
			$pdf->SetFont('Arial','B',14);
			$pdf->Cell(125,8,"",11);
			$pdf->Cell(100,8,"Lista de Productos Inactivos",0);
			$pdf->Ln(23);
			$pdf->SetFont('Arial','B',12);

		$pdf->SetWidths(array(65,35,35,25));
		$pdf->Cell(70,60,"",0);

	$pdf->Cell(65,8,"Nombre",'TRLB',0);
	$pdf->Cell(35,8,"Precio",'TRLB',0);
	$pdf->Cell(35,8,"Fecha de Alta",'TRLB',0);
	$pdf->Cell(25,8,"Estado",'TRLB',0);
	$pdf->Ln(8);
	$pdf->SetFont("Arial",'',12);
	...
	$controlador = new Producto();
		...
		foreach($sql as $row){ 
				$pdf->Cell(70,60,"",0);
    $pdf->Row(array($row['ProductoNombre'],$row['ProductoPrecio'],$row['ProductoFechaAltaDB'],$row['ProductoEstado']));
